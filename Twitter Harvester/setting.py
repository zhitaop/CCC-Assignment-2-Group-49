'''
TEAM 49
Member:
'''

VIC_BOX = [
    140.96,
    -39.2,
    150.03,
    -33.98
    ]

DB_NAME = 'tweet'

COUCHDB_ADDRESS = 'http://127.0.0.1:5984/'

AUTH_LIST = [
    #index = 0, Lu1
    {
        'rest_ck' : 'A5KRagrcPWbwUF7UkguySO5au',
        'rest_cs' : 'yfKVPUWHrrtVmwvBzrGecZvBvISQwZ48xNkgvfweN8dYt3lmES',
        'rest_at' : '1126754709421715458-Ttxz0zua58zI9EIAAVEuXgmX6c0m2F',
        'rest_as' : 'PHQ1CZBLPP5xHNHr8Uvt0mLpzUrgoqojDJcc2Z6j5tHJi'
    },
    #index = 1, Han1
    {
        'rest_ck' : 'ovzZpAxizMf40aRBebETy65vR',
        'rest_cs' : '4UR8d3JoAN6SNYblgq8XtV0xdsutv4MPawAqI9Ob7yyahfX65l',
        'rest_at' : '967777720150052865-X2fTRFP6uauYsqrvE12aDewhuIXNNEf',
        'rest_as' : 'z1O9hIYt8QKps2I73ZjrFAvWIq8m9ZsVkAUStUVw2kSXX'
    },
    #index = 2, Pan1
    {
        'rest_ck' : 'Sds1H0cEaS7Vcg3UKgNJgXmjP',
        'rest_cs' : 'YQBjCfutKAEt1bfSiHbTXigPSyebFhg229t7UMJnQWMrbk1i0t',
        'rest_at' : '1059274882830090240-hwuhl4Ur2TmXTQjhZqEW0MGARyhRAo',
        'rest_as' : 'TBraXyUXTIbEGUMqvsULAmyPqqVuyrRoekEVo8mZuqQkG'
    },
    #index = 3, Zhang1
    {
        'rest_ck' : 'y0iZXfr8ZDCi9tmSlNZ33NPTa',
        'rest_cs' : 'BhBLtViYSG1v6RCyzVQ5h2epfgPx7vrQ4lALW4L6QZFSs7cj1i',
        'rest_at' : '987483594338717697-7SwKfgTD1H3YCKCPVxtvuGMS88n0zMX',
        'rest_as' : 'P9qdrdcMIdnM6BoP8n9cFndPd2B8y3dCpRH36GCBUlRC7'
    },
    #index = 4, Yang2
    {
        'rest_ck' : '9g47vtbaoLrL6UdZpalP3wziI',
        'rest_cs' : 'It8TB4S5WpcZTem8eK1a3yh3MuW49LIJgQcUT1E8VfMbO0JdOJ',
        'rest_at' : '1127480596337266688-5rBAkBZS5u5nQWq9JrhNeYMVFDt7ro',
        'rest_as' : '8cYbgEQUvcFdesxpiojDVp2ZqP9UPqnzDiSF2KjX7wGJ5'
    },
    #index = 5, Lu2
    {
        'rest_ck' : 'pzbMllfHPgKnKP8F8y6CXqrM6',
        'rest_cs' : 'va1IxRVmE4kVRIvhXyiIGryK5QJ7XXhMzKPSBYz2YE1mG2wymG',
        'rest_at' : '1127478273984700418-ztAaCxAmDzToVuUsRtXT6XEIlnA8I5',
        'rest_as' : 'ezJ5l9QGSatNhYSBpZVwE3FtCGSZaVA4XtKdCWc6TwYvc'
    }
]

STREAMING_AUTH = {
    #Yang1
    'c_k' : 'YUUejarJDZo5vFIUcU0jyNPck',
    'c_s' : 'TPFJyMMj4oF4TBdxyurBUQdEly0Cq7j8AR5yKyfhjNZvj5vcMb',
    'a_t' : '885237433-NBxPYyvZXRPZMLyKGtWwOmgnSfIHzd6H8Y18gwAN',
    'a_s' : 'Llfml16Lc4X8rvXusKmt6sAfakvHM3jc65RkD2XylwOF3'
}